...
macro_rules! impl_repository {
    ($repo:ident, $struct_name:ty, $collection:expr) => {
        impl $repo {
            ...
            pub async fn find_by_id(&self, id: &str, database: &Database) -> Result<Option<$struct_name>> {
                let entity = database
                    .collection::<$struct_name>($collection)
            ...
            pub async fn search<T>(&self, database: &Database, filter: &T) -> Result<Collection<$struct_name>>
            where
            ...
            pub async fn search_slice<T>(&self, database: &Database, filter: &T) -> Result<Vec<$struct_name>>
            where
                T: IFilter + IPaginator,
            {
